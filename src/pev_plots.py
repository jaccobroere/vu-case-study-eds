...
for dname in tqdm(pev_dataset_list):
    ...
    nz_cols_spca, nz_loadings_spca, PEV_arr_spca = get_data_pev(
        X,
        n_components=N_COMPONENTS,
        verbose=VERBOSE,
        step_size=STEP_ALPHA,
        get_transform=get_spca,
    )
    nz_cols_gspca, nz_loadings_gspca, PEV_arr_gspca = get_data_pev(
        X,
        n_components=N_COMPONENTS,
        verbose=VERBOSE,
        step_size=STEP_L1,
        get_transform=get_gene_spca,
    )
    ...
    logging_dict[dname] = {
        "spca": {
            "nz_cols": nz_cols_spca,
            "nz_loadings": nz_loadings_spca,
            "PEV_arr": PEV_arr_spca,
        },
        "gspca": {
            "nz_cols": nz_cols_gspca,
            "nz_loadings": nz_loadings_gspca,
            "PEV_arr": PEV_arr_gspca,
        },
    }
